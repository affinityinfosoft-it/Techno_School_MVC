...
{
    public class StudentDueFeeController : ApiController
    {
        public Service service = new Service();

        [HttpPost]
        public IHttpActionResult GetStudentDueFee(StudentDueFeeRequest obj)
        {
            ResultWithData<StudentDueFeeResponse> Result = new ResultWithData<StudentDueFeeResponse>();
            if (User.Identity.IsAuthenticated)
            {
                ...
                try
                {
                    ...
                    var data = service.GetStudentDueFee(obj);
                    if (data.Count() > 0)
                    {
