...
{
    public class StudentLiveClassController : ApiController
    {
        public Service service = new Service();

        [HttpPost]
        public IHttpActionResult GetStudentLiveClass(StudentLiveClassRequest obj)
        {
            ResultWithData<StudentLiveClassResponse> Result = new ResultWithData<StudentLiveClassResponse>();
            if (User.Identity.IsAuthenticated)
            {
                ...
                try
                {
                    ...
                    var data = service.GetStudentLiveClass(obj);
                    if (data.Count() > 0)
                    {
        ...
        [HttpPost]
        public IHttpActionResult GetStudentWiseFacultyList(StudentWiseFacultyRequest obj)
        {
            ResultWithData<StudentWiseFacultyResponse> Result = new ResultWithData<StudentWiseFacultyResponse>();
            if (User.Identity.IsAuthenticated)
            {
                var identity = User.Identity as ClaimsIdentity;
                if (identity != null)
                ...
                try
                {
                    var data = service.GetStudentWiseFacultyList(obj);
                    if (data != null)
                    {
                        Result.IsValid = true;
                        Result.List = data;
