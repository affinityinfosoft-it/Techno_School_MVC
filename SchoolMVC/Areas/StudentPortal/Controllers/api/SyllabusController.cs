...
namespace SchoolMVC.Areas.StudentPortal.Controllers.api
{
    public class SyllabusController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult GetSyllabus(SyllabusRequest obj)
        {
            ResultWithData<SyllabusResponse> Result = new ResultWithData<SyllabusResponse>();
            if (User.Identity.IsAuthenticated)
            {
                ...
                try
                {
                    ...
                    var data = service.GetSyllabus(obj);
                    if (data.Count()>0)
                    {
