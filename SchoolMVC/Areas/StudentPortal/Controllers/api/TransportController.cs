...
namespace SchoolMVC.Areas.StudentPortal.Controllers.api
{
    public class TransportController : ApiController
    {
        ...
        [HttpPost]
        public IHttpActionResult GetTransportDetails(TransportDetailsRequest obj)
        {
            ResultWithData<TransportDetailsResponse> Result = new ResultWithData<TransportDetailsResponse>();
            if (User.Identity.IsAuthenticated)
            {
                ...
                try
                {
                    var data = service.GetTransportDetails(obj);
                    if (data != null)
                    {
                        Result.IsValid = true;
                        Result.List = data;
