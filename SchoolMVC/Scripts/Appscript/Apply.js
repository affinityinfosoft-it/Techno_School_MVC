...
// SCHOOL DROP DOWN
function LoadSchoolDdl() {
    GetScoolDdl(true)
        .then(function (response) {
            PopulateSchool(response);
        })
        .fail(function () {
            alert("Unable to load school list");
        });
}

function GetScoolDdl(isAsync) {
   ...
   .fail(function () {
       d1.reject("Failed to load school list");
   });

   return d1.promise();
}

function PopulateSchool(schoolList) {

    var ddlSchool = $("#ddlSchool");
    ddlSchool.empty();

    ddlSchool.append(
        $('<option>', { value: "0", text: "Select School" })
    );
    ...
    if (schoolList && schoolList.length > 0) {
        $.each(schoolList, function (i, school) {
            ddlSchool.append(
                $('<option>', {
                    value: school.SCM_SCHOOLID,
                    text: school.SCM_SCHOOLNAME
                })
            );
        });
    }
 ...
 //GET CLASS DROP DOWN
function LoadClassDdl(schoolId) {
    GetClassDdl(schoolId, true)
        .then(function (response) {
            PopulateClass(response);
        })
        .fail(function () {
            alert("Unable to load class list");
        });
}

function GetClassDdl(schoolId, isAsync) {
    var d1 = $.Deferred();
    $.ajax({
        type: "GET",
        timeout: 30000,
        url: '/JQuery/GetClassDdl',
    ...
    .fail(function () {
        d1.reject("Failed to load class list");
    });
    return d1.promise();
}

function PopulateClass(classList) {

    var ddlClass = $("#ddlClass");
    ddlClass.empty();

    ddlClass.append(
        $('<option>', { value: "0", text: "Select Class" })
    );

    if (classList && classList.length > 0) {
        $.each(classList, function (i, Class) {
            ddlClass.append(
                $('<option>', {
                    value: Class.CM_CLASSID,
                    text: Class.CM_CLASSNAME
                })
            );
        });
    }
...
function PopulateClass(classList) {
    ...
    if (classList && classList.length > 0) {
        $.each(classList, function (i, cls) {
            ddlClass.append(
                $('<option>', {
                    value: cls.CM_CLASSID,
                    text: cls.CM_CLASSNAME
                })
            );
        });
    }
...
var stateDistricts = {
    ...
    "LA": ["Leh", "Kargil", "Drass", "Nubra", "Sham", "Zanskar", "Changthang"],
    "LD": ["Lakshadweep"],
...
$('#state').change(function () {
    ...
    districtDropdown.prop('disabled', false).empty();
    districtDropdown.append('<option value="" selected>Select District</option>');

    if (stateDistricts[stateCode]) {
        $.each(stateDistricts[stateCode], function (index, district) {
