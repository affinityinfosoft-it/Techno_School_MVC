function InsertUpdateRoute() {
    if (ValidateOperation() == true) {
        var _data = JSON.stringify({
            RT_ROUTEID: $('#RT_ROUTEID').val(),
            RT_ROUTENAME: $('#RT_ROUTENAME').val(),
            RT_LOCATION: $('#RT_LOCATION').val(),
            Userid: $('#hdnUserid').val()

        });

        $.ajax({
            url: '/JQuery/InsertUpdateRoute',
            data: _data,
            type: 'POST',
            dataType: 'json',
...
function BindList() {
    ...
    setTimeout(function () {
        //papai
        $.ajax({
            url: '/JQuery/RouteList',
            dataType: 'json',
            type: 'GET',
            success: function (res) {
                if (res.Data.length > 0) {
                    $data = $('<table id="tblList"></table>').addClass('table table-bordered table-striped');
                    $header = "<thead><tr><th style=display:none>Id</th><th>Serial No.</th><th>Route Name</th><th></th><th></th></tr></thead>";
                    $data.append($header);

                    $.each(res.Data, function (i, row) {
                        var $row = $('<tr/>');
                        $row.append($('<td style=display:none>').append(row.RT_ROUTEID));
                        $row.append($('<td>').append(i + 1));
                        $row.append($('<td>').append(row.RT_ROUTENAME));
                        if (res.CanEdit == true) {
                            $row.append($('<td>').append("<a href=/Masters/BusRoute/" + row.RT_ROUTEID + " class='btn btn-warning'>Edit</a>"));
                        }
                        else {
                            $row.append($('<td>').append("<a href=# class='btn btn-warning disabled'>Edit</a>"));
                        }
                        if (res.CanDelete == true) {
                            $row.append($('<td>').append("<a onclick='Confirm(" + row.RT_ROUTEID + ");'" + row.RT_ROUTEID + " class='btn btn-danger'>Delete</a>"));
                        }
                        else {
